...
/* JCAP Subtitle */
#define GST_MTS_DESC_ISDB_DATA_COMPONENT 0xFD
...
*  length
*  subtitle(){
*    ISO639_language_code   24 bslbf
*    subtitling_type        8 uimsbf
*    composition_page_id    16 uimsbf
*    ancillart_page_id      16 uimsbf
*  }
* }
...
gst_ts_demux_set_caps_for_private_subtitle (TSDemuxStream * stream,
    GstCaps * caps, guint8 * desc)
{
  gchar *lang = NULL;
  guint8 subtitling_type;
  guint16 composition_page_id, ancillart_page_id;
  int length;
  gchar lang_code[4];
  ...
  switch (desc[0]) {
    case GST_MTS_DESC_DVB_SUBTITLING:
      ...
      subtitling_type = desc[5];
      composition_page_id = GST_READ_UINT16_BE (desc + 6);
      ancillart_page_id = GST_READ_UINT16_BE (desc + 8);
      ...
      lang = g_malloc (strlen (lang_code) + 1);
      memcpy (lang, lang_code, strlen (lang_code) + 1);

      gst_caps_set_simple (caps, "pestype", G_TYPE_STRING, "DVB", NULL);
      gst_caps_set_simple (caps, "sublangcode", G_TYPE_STRING, lang, NULL);
      gst_caps_set_simple (caps, "subtitlingType", G_TYPE_UINT, subtitling_type,
          NULL);
      gst_caps_set_simple (caps, "compositionPageId", G_TYPE_UINT,
          composition_page_id, NULL);
      gst_caps_set_simple (caps, "ancillaryPageId", G_TYPE_UINT,
          ancillart_page_id, NULL);
      break;
...
gst_ts_demux_get_audio_type (TSDemuxStream * stream)
{
  ...
  if (desc) {
    ...
    ret = DESC_ISO_639_LANGUAGE_audio_type_nth (desc, 0);
    g_free (desc);
  }
  return ret;
}
    ...
    MpegTSBaseProgram * program)
{
  ...
  switch (bstream->stream_type) {
    ...
    case GST_MPEG_TS_STREAM_TYPE_VIDEO_MPEG2:
    {
      ...
      caps = gst_caps_new_simple ("video/mpeg",
          ...
          bstream->stream_type ==
          GST_MPEG_TS_STREAM_TYPE_VIDEO_MPEG1 ? "mp1v" : "mp2v", NULL);
    ...
    case GST_MPEG_TS_STREAM_TYPE_AUDIO_MPEG2:
      ...
      codec_name = g_strdup_printf ("MPEG-%d audio",
          bstream->stream_type == GST_MPEG_TS_STREAM_TYPE_AUDIO_MPEG1 ? 1 : 2);
      break;
    case GST_MPEG_TS_STREAM_TYPE_PRIVATE_PES_PACKETS:
    {
      ...
      if (desc) {
        ...
        gst_ts_demux_set_caps_for_private_subtitle (stream, caps, desc);
        break;
      }
      ...
      if (program->program_number == 10510 && bstream->pid == 3401) {
        ...
        caps = gst_caps_new_simple ("video/x-h264",
            "stream-format", G_TYPE_STRING, "byte-stream",
            "alignment", G_TYPE_STRING, "nal", "format", G_TYPE_STRING, "h264",
            NULL);
        tag_name = GST_TAG_VIDEO_CODEC;
    ...
    case GST_MPEG_TS_STREAM_TYPE_VIDEO_H264:
      ...
      break;
    case GST_MPEG_TS_STREAM_TYPE_VIDEO_H265:
    case GST_MPEG_TS_STREAM_TYPE_VIDEO_H265_HEVC:
      template = gst_static_pad_template_get (&video_template);
      name = g_strdup_printf ("video_%04x", bstream->pid);
      caps = gst_caps_new_simple ("video/x-h265",
          "stream-format", G_TYPE_STRING,
          "byte-stream", "alignment", G_TYPE_STRING,
          "nal", "format", G_TYPE_STRING, "h265", NULL);
      tag_name = GST_TAG_VIDEO_CODEC;
      codec_name = g_strdup ("ITU H.265");
      break;
    case ST_VIDEO_DIRAC:
  ...
  if (gst_tag_list_get_string (stream->taglist, GST_TAG_LANGUAGE_NAME,
          &lang_code)) {
  ...
  if (template && name && caps) {
    ...
    if (desc) {
      ctags = DESC_DVB_STREAM_IDENTIFIER_component_tag (desc);
    ...
    GST_INFO_OBJECT (caps,
        "stream:%p creating pad with name %s and caps %" GST_PTR_FORMAT, stream,
        name, caps);
...
activate_pad_for_stream (GstTSDemux * tsdemux, TSDemuxStream * stream)
{
  ...
  if (stream->pad) {
    ...
    if (alldone) {
      GST_INFO_OBJECT (tsdemux, "All pads were activated, emit no-more-pads");
    ...
    const guint8 * private_data)
{
  ...
  g_return_val_if_fail (!(GST_READ_UINT16_BE (private_data) & 0xFFF8), FALSE);
  g_return_val_if_fail (!(GST_READ_UINT16_BE (private_data) & 0xFFE0), FALSE);

  /* marker bits check. It has a value of '1' */
  for (i = 0; i < 16; i += 2) {
    g_return_val_if_fail (GST_READ_UINT16_BE (private_data + i) & 0x0001,
        FALSE);
  }
...
calculate_and_push_newsegment (GstTSDemux * demux, TSDemuxStream * stream)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (lowest_pts)) {
    firstts = lowest_pts;
    GST_DEBUG ("lowest_pts %" G_GUINT64_FORMAT " => clocktime %"
...
gst_ts_demux_push_pending_data (GstTSDemux * demux, TSDemuxStream * stream)
{
  ...
  GST_INFO_OBJECT (stream->pad, "stream->pts %" GST_TIME_FORMAT,
  ...
  if (demux->segment.start > GST_BUFFER_PTS (buffer)) {
    GST_INFO_OBJECT (stream->pad,
        "Drop: segment.start: %" GST_TIME_FORMAT " , buffer_start: %"
        GST_TIME_FORMAT, GST_TIME_ARGS (demux->segment.start),
        GST_TIME_ARGS (GST_BUFFER_PTS (buffer)));
    GST_INFO_OBJECT (stream->pad, "Detected bad timestamp!!!");
...
{
  GST_DEBUG_CATEGORY_INIT (ts_demux_debug, "skbmpegtsdemux", 0,
      "MPEG transport stream demuxer for SKB");
  init_pes_parser ();

  return gst_element_register (plugin, "skbmpegtsdemux",
