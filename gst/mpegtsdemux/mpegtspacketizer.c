    ...
    guint64 pcr, guint64 offset)
{
  ...
  if (pcrtable->last_pcr == -1 || pcrtable->last_offset < offset) {
    ...
    priv->nb_seen_offsets++;
    priv->last_pcr_offsets = pcrtable->last_offset;
  }
}
...
guint64
mpegts_packetizer_get_last_pcr (MpegTSPacketizer2 * packetizer)
{
  return packetizer->priv->last_pcr_offsets;
}
    ...
    GstClockTime pts, guint16 pcr_pid)
{
  ...
  if (packetizer->calculate_offset && pcrtable->first_pcr != -1) {
    ...
    if ((G_UNLIKELY (pts < pcrtable->first_pcr_ts)) &&
        (GST_CLOCK_DIFF (pts, pcrtable->first_pcr_ts) > (GST_SECOND * 2)))
      pts += MPEGTIME_TO_GSTTIME (PTS_DTS_MAX_VALUE);
    if (pts >= pcrtable->first_pcr_ts)
...
/* Some CPs are using "stop" and "play" event, when it resume or seek play.
  * So we must use real-time.
  */
GstClockTime
