    ...
    guint16 new_pmt_pid, const GstMpegTsPMT * new_pmt)
{
  ...
  /* We don't need to check the below info.
   * Because the pipeline can't handle the pad changing. */
#if 0
  /* Check the streams */
...
mpegts_base_scan (MpegTSBase * base)
{
  ...
  /*arun.s for file duration fix */
  guint seekback;
  guint64 seekstop, last_pcr_offset, firstseekpos;
  gboolean last_pcr;
  ...
  /* arun.s for file duration fix */
  last_pcr = FALSE;
  seekstop = upstream_size;
  for (seekback = 1; !last_pcr; seekback++) {
    firstseekpos = MAX (0, upstream_size - seekback * 655360);
    /* Find last PCR value */
    for (seek_pos = MAX (0, upstream_size - seekback * 655360);
        seek_pos < seekstop && !done; seek_pos += 65536) {
    ...
    /*
     * arun.s for calculating proper file duration
     * if PCR not found at end,scan till we find
     * atleast one valid PCR
     */
    last_pcr_offset = mpegts_packetizer_get_last_pcr (base->packetizer);
    if (last_pcr_offset < upstream_size / 2) {
      seekstop = firstseekpos;
    } else {
      last_pcr = TRUE;
    }
  }
...
{
  GST_DEBUG_CATEGORY_INIT (mpegts_base_debug, "skbmpegtsbase", 0,
      "MPEG transport stream base class for SKB");
